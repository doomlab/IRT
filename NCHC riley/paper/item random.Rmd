...
####pil 
setwd("~/Desktop")
master = read.csv("Meaning_Scales_PIL_RR_RN_Paper.csv") ##or however we want to import this
...
##replace what we should
replacepeople = notypos[ missing <= 5 , ]  
dontpeople = notypos[ missing > 5 , ]

##drop survey data column 
...
library(lavaan)
overallmodel = '
PIL =~Q29_1 + Q31_1 + Q33_1 + Q34_1 + Q35_1 + Q36_1 + Q37_1 + Q38_1 + Q39_1 + Q40_1 + Q41_1 + Q42_1 + Q43_1 + Q44_1 + Q45_1 + Q46_1 + Q47_1 + Q48_1 + Q49_1 + Q50_1
'
...
####lpq 
setwd("~/Desktop")
master = read.csv("Meaning_Scales_LifePurposeQuestionnaire_RR_RN_Paper.csv") ##or however we want to import this

##reverse code (true is 1, false is 2)
...
fitmeasures(partialstrict$fit.configural)
...
YOU CAN WRITE ALPHA LIKE $\alpha$
...
cortest.mat(cor_notrandomP, cor_randomP, 
...
sum(abs(z_differenceP) > 1.96, na.rm = T) / 2


```

IF YOU DO NOT CHANGE THIS WHOLE SECTION BE SURE TO TALK ABOUT PIL 16 IS THE OTHER ONE WE MISSED.  
...
LPQ total scores were created by summing the items for each participant. The mean total score for randomized testing was *M* = `r apa(MTrL, 2, F)` (*SD* = `r apa(SDTrL, 2, F)`), with good reliability (\alpha = 0.82), and the mean for nonrandomized testing was *M* = `r apa(MTnrL, 2, F)` (*SD* = `r apa(SDTnrL, 2, F)`) and good reliability (\alpha = 0.84). An independent *t*-test indicated that the testing did not change total score, `r apa_print(TLt)$statistic`. The effect size for this difference was negligible, $d_{av}$ = `r apa(TLd$d, 2)` 95% CI [`r apa(TLd$dlow, 2)`, `r apa(ILd$dhigh, 2)`]. The TOST test indicated that the scores were withing a 5% (`r apa(totcutoff, 2)` points) change, lower: *t*(1627) = `r apa(TLtost$TOST_t1, 2)`, *p* < .001 and upper: *t*(1627) = `r apa(TLtost$TOST_t2, 2)`, *p* < .001. The BF results were in support of the null model, 0.06 ±0.04%.
