...
## models
bpsf_mg <- equal_test(overall.model, bp_dssf$fulldata, .10)
...
summary_mg[nrow(summary_mg)+1, ] <- 
  ...
  c("BP", length(elg_mg$overall@Data@ov.names[[1]]), 
    "Overall", elq_mg$overall@SampleStats@nobs[[1]], 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(elq_mg$overall, c("chisq", "df", "cfi", "rmsea", "srmr")),
    "NA")

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(elg_mg$overall@Data@ov.names[[1]]),
    "Random", elq_mg$random@SampleStats@nobs[[1]], 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(elq_mg$random, c("chisq", "df", "cfi", "rmsea", "srmr")),
    elq_mg$random.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(elq_mg$overall@Data@ov.names[[1]]),
    "NonRandom", elq_mg$nonrandom@SampleStats@nobs[[1]], 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(elq_mg$nonrandom, c("chisq", "df", "cfi", "rmsea", "srmr")),
   elq_mg$nonrandom.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(elq_mg$overall@Data@ov.names[[1]]),
    "Configural", sum(unlist(elq_mg$configural@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(elq_mg$configural, c("chisq", "df", "cfi", "rmsea", "srmr")),
    "NA")

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(elq_mg$overall@Data@ov.names[[1]]), 
    "Metric", sum(unlist(elq_mg$metric@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(elq_mg$metric, c("chisq", "df", "cfi", "rmsea", "srmr")),
   elq_mg$metric.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(elq_mg$overall@Data@ov.names[[1]]),
    "Scalar", sum(unlist(elq_mg$scalar@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(elq_mg$scalar, c("chisq", "df", "cfi", "rmsea", "srmr")),
   elq_mg$scalar.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(elq_mg$overall@Data@ov.names[[1]]),
    "Strict", sum(unlist(elq_mg$residuals@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(elq_mg$residuals, c("chisq", "df", "cfi", "rmsea", "srmr")),
    elq_mg$residuals.equal$pval)
```
