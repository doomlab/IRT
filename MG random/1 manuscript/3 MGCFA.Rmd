...
# Model Code
overallmodel = '
GLPS =~ GLPS1 + GLPS2 + GLPS3 + GLPS4 + GLPS5 + GLPS6 + GLPS7 + GLPS8 + GLPS9 + GLPS10 + GLPS11 + GLPS12 + GLPS13 + GLPS14 + GLPS15
'
...
####NOTE - did we say to remove social desirability? we actually have these questions. It is coded wrong in the master scale table file ;) note some items missing because inappropriate factor loadings (see doc of the measure). also see cv label on measure which does not correspond to a factor, also see the last two items. need some clarity on this one 
```{r mg_lapr, include = F}
# Model Code
overall.model = '
PM =~ LAPR + LAPR2 + LAPR5 + LAPR7 + LAPR12 + LAPR16 + LAPR20 + LAPR27 + LAPR29 + LAPR35 + LAPR37 + LAPR38 + LAPR48 
EV =~ LAPR4 + LAPR9 + LAPR33 
DA =~ LAPR8 + LAPR22 + LAPR25 + LAPR28 + LAPR32 + LAPR44 + LAPR47 
GS =~ LAPR10 + LAPR14 + LAPR24 + LAPR34 + LAPR36 + LAPR41 + LAPR43
CR =~ LAPR3 + LAPR11 + LAPR17 + LAPR19 + LAPR23 + LAPR30 + LAPR39 + LAPR40 + LAPR42 + LAPR45 
SD =~ LAPR15 + LAPR18 + LAPR21 + LAPR26 + LAPR31 + LAPR46 + LAPR49 + LAPR50 + LAPR51 + LAPR52 + LAPR53 + LAPR54
'
...
# Model Code
overall.model = '
LPQ =~ LPQ1 + LPQ2 + LPQ3 + LPQ4 + LPQ5 + LPQ6 + LPQ7 + LPQ8 + LPQ9 + LPQ10 + LPQ11 + LPQ12 + LPQ13 + LPQ14 + LPQ15 + LPQ16 + LPQ17 + LPQ18 + LPQ19 + LPQ20
'
...
# Model Code
overall.model = '
MAPA =~ MAPA1 + MAPA2 + MAPA3 + MAPA4 + MAPA5 + MAPA6 + MAPA7 + MAPA8 + MAPA9 + MAPA10 + MAPA11 + MAPA12 + MAPA13 + MAPA14 + MAPA15 + MAPA16 + MAPA17 + MAPA18 + MAPA19 + MAPA20 + MAPA21 + MAPA22 + MAPA23 + MAPA24 + MAPA25 + MAPA26 + MAPA27 + MAPA28
'
...
- listed as kearnes but really it's steger, so run it that way 
...
# Model Code
overall.model = '
SOMP =~ SOMP1 + SOMP2 + SOMP3 + SOMP4 + SOMP5 + SOMP6 + SOMP7 + SOMP8 + SOMP9 + SOMP10 + SOMP11 + SOMP12 + SOMP13
'

## models
somp_mg <- equal_test(overall.model, somp_ds$fulldata, .10)

## Put that into the table 
summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(somp_mg$overall@Data@ov.names[[1]]), 
    "Overall", somp_mg$overall@SampleStats@nobs[[1]], 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(somp_mg$overall, c("chisq", "df", "cfi", "rmsea", "srmr")),
    rep("NA", 4))

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(somp_mg$overall@Data@ov.names[[1]]),
    "Random", somp_mg$random@SampleStats@nobs[[1]], 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(somp_mg$random, c("chisq", "df", "cfi", "rmsea", "srmr")),
    somp_mg$random.equal$chi,
    fitmeasures(somp_mg$random, "df"),
    somp_mg$random.equal$popdelt,
    somp_mg$random.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(somp_mg$overall@Data@ov.names[[1]]),
    "NonRandom", somp_mg$nonrandom@SampleStats@nobs[[1]], 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(somp_mg$nonrandom, c("chisq", "df", "cfi", "rmsea", "srmr")),
    somp_mg$nonrandom.equal$chi,
    fitmeasures(somp_mg$nonrandom, "df"),
    somp_mg$nonrandom.equal$popdelt,
    somp_mg$nonrandom.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(somp_mg$overall@Data@ov.names[[1]]),
    "Configural", sum(unlist(somp_mg$configural@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(somp_mg$configural, c("chisq", "df", "cfi", "rmsea", "srmr")),
    rep("NA", 4))

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(somp_mg$overall@Data@ov.names[[1]]), 
    "Metric", sum(unlist(somp_mg$metric@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(somp_mg$metric, c("chisq", "df", "cfi", "rmsea", "srmr")),
    somp_mg$metric.equal$chi,
    abs(fitmeasures(somp_mg$metric, "df") - fitmeasures(somp_mg$configural, "df")),
    somp_mg$metric.equal$popdelt,
    somp_mg$metric.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(somp_mg$overall@Data@ov.names[[1]]),
    "Scalar", sum(unlist(somp_mg$scalar@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(somp_mg$scalar, c("chisq", "df", "cfi", "rmsea", "srmr")),
    somp_mg$scalar.equal$chi,
    abs(fitmeasures(somp_mg$scalar, "df") - fitmeasures(somp_mg$metric, "df")),
    somp_mg$scalar.equal$popdelt,
    somp_mg$scalar.equal$pval)

summary_mg[nrow(summary_mg)+1, ] <- 
  #scale name, model step, sample size
  c("BP", length(somp_mg$overall@Data@ov.names[[1]]),
    "Strict", sum(unlist(somp_mg$residuals@SampleStats@nobs)), 
    #chisq, df, cfi, rmsea, srmr
    fitmeasures(somp_mg$residuals, c("chisq", "df", "cfi", "rmsea", "srmr")),
    somp_mg$residuals.equal$chi,
    abs(fitmeasures(somp_mg$residuals, "df") - fitmeasures(somp_mg$scalar, "df")),
    somp_mg$residuals.equal$popdelt,
    somp_mg$residuals.equal$pval)
...
####Note used VIL items, there were 42 on the PDF but we only had 38 items. any reason? coded the 38, otherwise we are good couldnt remember if we discussed this or not 

```{r mg_stms, include = F}
# Model Code
overall.model = '
LC =~ VIL37 + VIL36 + VIL20 + VIL26 + VIL6 + VIL38 + VIL35 + VIL28 + VIL33 + VIL10
PMIL =~ VIL15 + VIL13 + VIL17 + VIL11 + VIL31 + VIL23 + VIL27 + VIL29 
NMIL =~ VIL8 + VIL4 + VIL5 + VIL3 + VIL1 + VIL9 + VIL7 + VIL2
WTM =~ VIL32 + VIL18 + VIL34 + VIL22 + VIL30 + VIL14 + VIL16 + VIL12
'
