...
summary_ds[nrow(summary_ds)+1, ] <- 
                    ...
                    c("BP", sum(!is.na(bpsf_df$fulldata$outliers)),
                      # missing points, number outliers
                      bpsf_df$missing_points, bpsf_df$num_outliers, 
                      # additivity
                      sum(abs(bpsf_df$correl) > .90 & abs(bpsf_df$correl) < 1), 
                      # linearity, normality, homog, homos
                      "Yes", "No", "Yes", "Yes")
```

```{r mg_brief_resilience_scale}
# Data
## Expression is partno | (or) put in the letters you want, leave numbers
...
summary_ds[nrow(summary_ds)+1, ] <- 
                    ...
                    c("BP", sum(!is.na(brs_df$fulldata$outliers)),
                      # missing points, number outliers
                      brs_df$missing_points, brs_df$num_outliers, 
                      # additivity
                      sum(abs(brs_df$correl) > .90 & abs(brs_df$correl) < 1), 
                      # linearity, normality, homog, homos
                      "No", "No", "No", "No")
```